...
function useStoryPageTexture(book: StoryBook, page?: StoryBookPage, side: "left" | "right" | "single" = "left") {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      const pageLabel = page?.pageNumber ? `Page ${page.pageNumber}` : isMoral ? "Morale" : "Page";
      ...
      if (isLocked) {
        ...
        wrapText(
          ctx,
          page?.text ??
            "Débloque le mini-livre complet pour lire les 8 pages, le garder en PDF et le retrouver quand tu veux.",
          190,
          780,
          820,
          58,
        ...
        ctx.textAlign = "center";
        ctx.fillText("Débloquer le livre — 4,99 €", 600, 1088);
        ...
        ctx.fillStyle = "#070b2d";
        ctx.font = "900 32px Arial";
        ctx.fillText("Ajouter l’audio — 7,99 €", 600, 1202);
        ctx.textAlign = "left";
      ...
      const textY = pageImage ? 855 : isMoral ? 430 : 330;
      ctx.fillStyle = isMoral ? "#6b55ef" : "#070b2d";
      ctx.font = isMoral ? "italic 800 86px Georgia" : "800 60px Arial";
      wrapText(ctx, page?.title ?? book.title, 96, textY, 1008, isMoral ? 94 : 74, 3);

      ctx.fillStyle = "#4c5578";
      ctx.font = isMoral ? "700 46px Arial" : "700 41px Arial";
      wrapText(ctx, page?.text ?? "", 96, textY + (isMoral ? 250 : 150), 1008, 66, isMoral ? 6 : 7);
